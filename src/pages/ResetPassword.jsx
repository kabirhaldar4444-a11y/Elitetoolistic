...
const ResetPassword = () => {
  ...
  useEffect(() => {
    // Listen for auth state changes to catch the session from the recovery link
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === 'PASSWORD_RECOVERY') {
        // This event specifically triggers when the user arrives from a recovery link
        console.log('Recovery session detected');
      }

      if (!session && event !== 'INITIAL_SESSION') {
        toast('Invalid or expired recovery link. Please request a new one.', 'error');
        navigate('/login');
      }
    });
    ...
    const checkInitialSession = async () => {
      ...
      if (!session) {
        // Give it a moment to process the hash token if it's there
        setTimeout(async () => {
          const { data: { session: retrySession } } = await supabase.auth.getSession();
          if (!retrySession) {
            toast('Session expired or invalid link. Please retry recovery.', 'error');
            navigate('/login');
          }
        }, 1000);
      }
  ...
  const handleUpdate = async (e) => {
    ...
    if (password !== confirmPassword) {
      return toast('Passwords do not match. Please verify.', 'error');
    }
    ...
    try {
      ...
      // Post-Update Sign Out to ensure fresh login
      ...
      <div className="relative z-10 w-full max-w-[400px] px-6 animate-fade-in py-12">
        <div className="flex flex-col items-center mb-16">
          ...
          <div className="mt-8 text-center space-y-1">
            <h1 className="text-2xl font-black tracking-widest text-[#1e293b] uppercase font-serif">Security Reset</h1>
            <p className="text-[10px] font-black tracking-[0.4em] text-[#A51C30] uppercase">Update Your Access Key</p>
          </div>
        </div>
