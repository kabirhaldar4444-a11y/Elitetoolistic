...
const Login = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (email === 'support@harvardlearning.in' || profile.role === 'admin') {
      ...
      <div className="relative z-10 w-full max-w-[400px] px-6 animate-fade-in py-12">
        ...
        <div className="glass-card-saas p-10 !rounded-[2.5rem] shadow-2xl border-t-4 border-t-[#A51C30]">
          <form onSubmit={handleLogin} className="flex flex-col gap-8">
            ...
            <button
              type="button"
              onClick={() => navigate('/PasswordLogin')}
              className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-[#A51C30] transition-colors text-center w-full mt-2"
            >
